...
import type { VideoEvent } from "../shared/video.map.type";
...
const VideoEventTab = ({
  ...
  const eventTagStyle: React.CSSProperties = {
    ...
    top: firstMappingPosY + trackHeight * (videoEvent.trackIndex ?? 0),
...
const Timeline = ({
  ...
  assignEventMappings(videoEvents, timelineScale);
  ...
  const firstMappingPosY = segmentPosY + (segments.length <= 0 ? 0 : 40);
  const timelineHeight = firstMappingPosY + trackHeight * trackAmount;
  ...
  const videoEventComponents = videoEvents.map((videoEvent) => {
    ...
    return (
      <VideoEventTab
        ...
        firstMappingPosY={firstMappingPosY}

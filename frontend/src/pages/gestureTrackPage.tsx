...
import { GestureTrack, JediConfig } from "../shared/motion.track.type";
import {
  getGestureTracks,
  getJediConfig,
  postGestureTracks,
} from "../services/gestureTrackService";
...
export default function GestureTrackPage() {
  ...
  const [gestureTracks, setGestureTracks] = useState<GestureTrack[]>([]);
  const [jediConfig, setJediConfig] = useState<JediConfig | null>(null);

  useEffect(() => {
    (async () => {
      const res = await getGestureTracks();
      if (res.data) setGestureTracks(res.data);
  ...
  const handleSubmit = async () => {
    for (const entry of gestureTracks) {
    ...
    const res = await postGestureTracks(gestureTracks);

    if (res.data) {
      setGestureTracks(res.data);
      setMessage("Gesture track updated successfully.");
    }
    if (res.errorMsg) {
          ...
          page === "gesture-input" ? (
            ...
            <div className="flex-col">
              ...
              <table>
                ...
                <tbody>
                  {gestureTracks.map((track, idx) => (
                    <tr key={idx}>
                      <td className="gray">
                        {track.gesture === jediConfig?.start_event &&
                          "Event (Gesture) switch"}
                        {track.gesture === jediConfig?.start_live &&
                          "Live (Follower) switch"}
                        {track.gesture === jediConfig?.stop &&
                          "Off (Inactive) switch"}
                      </td>
                      <td>{track.gesture}</td>
                      <td>

...
const AUDIO_MAPPING_URL = "/api/audio-mapping";
...
export async function getAudioMapping(): Promise<APIResponse<MotionPayload>> {
  try {
    const res = await api.get<MotionPayload>(AUDIO_MAPPING_URL);
...
export async function saveAudioMapping(
  data: MotionPayload
): Promise<APIResponse<MotionPayload>> {
  try {
    const res = await api.post<MotionPayload>(AUDIO_MAPPING_URL, data);
    const parsed = motionPayloadSchema.parse(res.data);
    return {
      data: parsed,
    };
...
export async function getAudioFlip(): Promise<APIResponse<boolean>> {
  try {
    const res = await api.get<AudioFlipPayload>(`${AUDIO_MAPPING_URL}/flip`);
...
export async function toggleAudioFlip(): Promise<APIResponse<boolean>> {
  try {
    const res = await api.post<AudioFlipPayload>(`${AUDIO_MAPPING_URL}/flip`);
    const data = audioFlipPayloadSchema.parse(res.data);
    return {
      data: data.flip,
    };

...
import { MotionPayload as MotionType } from "../shared/motion.track.type";
...
const VideoTrackEditor = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const dataRes = await getVideoTrack(videoFileName);
      if (dataRes.data) {
        setVideoEventsReady(true);
        loadVideoTrack(dataRes.data);
      }
      if (dataRes.errorMsg) {
  ...
  const loadVideoTrack = async (data: VideoTrack) => {
  ...
  const saveVideoEvents = async (): Promise<void> => {
    ...
    const res = await postVideoTrack(data);
    if (res.data) {
  ...
  const selectVideo = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const dataRes = await getVideoTrack(name);
    if (dataRes.data) {
      setVideoEventsReady(true);
      loadVideoTrack(dataRes.data);
    }
  ...
  return (
    <div className="flex-col" tabIndex={0} onKeyDown={keyDownEvent}>
      ...
      {videoEventsReady ? (
        ...
        <>
          <div>Video Track has not been created.</div>
          <button onClick={saveVideoEvents}>Add Video Track</button>
        </>
      )}
    </div>
  );

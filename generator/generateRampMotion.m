function builtMotion = generateRampMotion(srcMotion)
    % GENERATERAMPMOTION  Generate a built motion for a ramp primitive.
    %   builtMotion = generateRampMotion(srcMotion) takes a source motion struct
    %   and produces a built motion struct with force sequences that ramp linearly
    %   from startValue to endValue over the duration.
    %
    %   Input:
    ...
    y = linspace(startValue, endValue, signalLen);    % Linear ramp from startValue to endValue
    ...
    builtMotion.shortDisplayName = 'Ramp';
    builtMotion.longDisplayName = sprintf('Ramp (%g to %g)', startValue, endValue);
